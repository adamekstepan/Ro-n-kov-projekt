...
if(isset($_POST['update_product'])){
   ...
   $message[] = 'produkt aktualizován!';

   if(!empty($image)){
      if($image_size > 2000000){
         $message[] = 'obrázek je velký!';
      }else{
         $update_image = $conn->prepare("UPDATE `products` SET image = ? WHERE id = ?");
         $update_image->execute([$image, $pid]);
         move_uploaded_file($image_tmp_name, $image_folder);
         unlink('uploaded_img/'.$old_image);
         $message[] = 'obrázek aktualizován!';
      }
   }
...
<section class="update-product">
   ...
   <h1 class="heading">upravit položku</h1>
   ...
   <form action="" enctype="multipart/form-data" method="post">
      ...
      <div class="flex-btn">
         <input type="submit" value="upravit" class="btn" name="update_product">
         <a href="admin_products.php" class="option-btn">zpět</a>
      </div>
   </form>
   ...
   <?php
         ...
         echo '<p class="empty">produkt nenalezen!</p>';
      }
   ?>
